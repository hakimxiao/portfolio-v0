...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--sunpillar-1': 'hsl(281, 92%, 68%)',
      '--sunpillar-2': 'hsl(258, 82%, 70%)',
      '--sunpillar-3': 'hsl(219, 92%, 70%)',
      '--sunpillar-4': 'hsl(191, 88%, 72%)',
      '--sunpillar-5': 'hsl(281, 92%, 68%)',
      '--sunpillar-6': 'hsl(258, 82%, 70%)',
  ...
  const shineStyle = {
    ...
    filter: 'brightness(0.72) contrast(1.15) saturate(0.45) opacity(0.28)',
  ...
  const glareStyle: React.CSSProperties = {
    ...
    backgroundImage: `radial-gradient(
      farthest-corner circle at var(--pointer-x) var(--pointer-y),
      rgba(190, 99, 249, 0.38) 8%,
      rgba(113, 196, 255, 0.2) 42%,
      rgba(0, 0, 0, 0.35) 92%
    )`,
    mixBlendMode: 'screen',
  ...
  return (
    ...
    >
      ...
      <div ref={shellRef} className="relative z-[1] group">
        <section
          className="grid relative overflow-hidden w-full max-w-full"
          style={{
            width: 'min(100%, calc(min(74svh, 500px) * 0.718))',
            height: 'auto',
            maxHeight: 'min(74svh, 500px)',
            aspectRatio: '0.718',
            borderRadius: cardRadius,
            ...
            background: 'linear-gradient(145deg, rgba(20, 20, 24, 0.98), rgba(8, 8, 8, 0.96))',
        ...
        >
          ...
          >
            ...
            >
              ...
              {showUserInfo && (
                <div
                  className="absolute z-[2] flex flex-col gap-3 backdrop-blur-[30px] border border-white/10 pointer-events-auto"
                  style={
                    {
                      '--ui-inset': 'clamp(16px, 6%, 24px)',
                      '--ui-radius-bias': '6px',
                      bottom: 'var(--ui-inset)',
                      left: 'var(--ui-inset)',
                      right: 'var(--ui-inset)',
                      background: 'linear-gradient(135deg, rgba(17, 17, 17, 0.88), rgba(35, 35, 35, 0.7))',
                      borderRadius: 'calc(max(0px, var(--card-radius) - var(--ui-inset) + var(--ui-radius-bias)))',
                      borderColor: 'rgba(190, 99, 249, 0.28)',
                      boxShadow: 'none',
                      padding: 'clamp(10px, 3.5%, 12px) clamp(10px, 4%, 14px)'
                    } as React.CSSProperties
                  }
                >
                  <dl
                    className="grid w-full gap-x-3 gap-y-2"
                    style={{ gridTemplateColumns: 'minmax(68px, 0.42fr) minmax(0, 1fr)' }}
                  >
                    {profileDetails.map(item => (
                      <React.Fragment key={item.label}>
                        <dt className="truncate text-[10px] font-semibold uppercase tracking-normal text-white/45">
                          {item.label}
                        </dt>
                        <dd className="truncate text-[11px] font-medium text-white/85">{item.value}</dd>
                      </React.Fragment>
                    ))}
                  </dl>
                </div>
          ...
          >
              <div className="w-full absolute flex flex-col" style={{ top: '3em', display: 'flex', gridArea: 'auto' }}>
                <h3
                  ...
                  style={{
                    fontSize: 'min(4.4svh, 2.6em)',
                    backgroundImage: 'linear-gradient(to bottom, #fff, #be63f9)',
